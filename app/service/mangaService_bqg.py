...
def get_home_manga():
    ...
    for category in categories:
        data = list(MangaSerialize(Manga.objects.filter(category__name=category.name)[:50], many=True).data)
        # 随机打乱
        random.shuffle(data)
        home_manga.append({
            "title": category.name,
...
def get_manga_info(manga_id, manga_url):
    ...
    if manga is None:
        data = bqgmh.get_manga_info(manga_url)
...
def get_chapter_list(manga_url):
    ...
    # 调用工具类获取所有章节
    lists = bqgmh.get_chapter_lists(manga_url)
    res["data"] = lists
    return res
...
def get_chapter_images(chapter_url):
    ...
    data = bqgmh.get_chapter_images(chapter_url)
    res["data"] = data
...
def manga_search(keywords, page):
    ...
    data = bqgmh.search(keywords, page)
